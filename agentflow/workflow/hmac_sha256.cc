#include "agentflow/workflow/hmac_sha256.h"

#include <array>
#include <cstdint>
#include <cstring>
#include <string>

namespace agentflow::workflow {
namespace {

// SHA-256 (public domain, adapted from Brad Conte's B-Con reference impl).

struct Sha256Ctx {
  uint8_t  data[64];
  uint32_t datalen;
  uint64_t bitlen;
  uint32_t state[8];
};

constexpr uint32_t kK[64] = {
  0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
  0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
  0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
  0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
  0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,
  0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
  0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,
  0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
};

inline uint32_t Rotr(uint32_t x, int n) { return (x >> n) | (x << (32 - n)); }

void Sha256Transform(Sha256Ctx& c, const uint8_t* d) {
  uint32_t m[64];
  for (int i = 0, j = 0; i < 16; ++i, j += 4) {
    m[i] = (uint32_t(d[j]) << 24) | (uint32_t(d[j+1]) << 16) |
           (uint32_t(d[j+2]) << 8) | uint32_t(d[j+3]);
  }
  for (int i = 16; i < 64; ++i) {
    uint32_t s0 = Rotr(m[i-15], 7) ^ Rotr(m[i-15], 18) ^ (m[i-15] >> 3);
    uint32_t s1 = Rotr(m[i-2], 17) ^ Rotr(m[i-2], 19) ^ (m[i-2] >> 10);
    m[i] = m[i-16] + s0 + m[i-7] + s1;
  }
  uint32_t a=c.state[0],b=c.state[1],cc=c.state[2],dd=c.state[3],
           e=c.state[4],f=c.state[5],g=c.state[6],h=c.state[7];
  for (int i = 0; i < 64; ++i) {
    uint32_t t1 = h + (Rotr(e,6)^Rotr(e,11)^Rotr(e,25)) + ((e&f)^(~e & g)) + kK[i] + m[i];
    uint32_t t2 = (Rotr(a,2)^Rotr(a,13)^Rotr(a,22)) + ((a&b)^(a&cc)^(b&cc));
    h=g; g=f; f=e; e=dd+t1; dd=cc; cc=b; b=a; a=t1+t2;
  }
  c.state[0]+=a; c.state[1]+=b; c.state[2]+=cc; c.state[3]+=dd;
  c.state[4]+=e; c.state[5]+=f; c.state[6]+=g; c.state[7]+=h;
}

void Sha256Init(Sha256Ctx& c) {
  c.datalen = 0; c.bitlen = 0;
  c.state[0]=0x6a09e667; c.state[1]=0xbb67ae85; c.state[2]=0x3c6ef372; c.state[3]=0xa54ff53a;
  c.state[4]=0x510e527f; c.state[5]=0x9b05688c; c.state[6]=0x1f83d9ab; c.state[7]=0x5be0cd19;
}

void Sha256Update(Sha256Ctx& c, const uint8_t* d, size_t len) {
  for (size_t i = 0; i < len; ++i) {
    c.data[c.datalen++] = d[i];
    if (c.datalen == 64) {
      Sha256Transform(c, c.data);
      c.bitlen += 512;
      c.datalen = 0;
    }
  }
}

void Sha256Final(Sha256Ctx& c, uint8_t* hash) {
  uint32_t i = c.datalen;
  if (c.datalen < 56) {
    c.data[i++] = 0x80;
    while (i < 56) c.data[i++] = 0;
  } else {
    c.data[i++] = 0x80;
    while (i < 64) c.data[i++] = 0;
    Sha256Transform(c, c.data);
    std::memset(c.data, 0, 56);
  }
  c.bitlen += c.datalen * 8;
  for (int k = 0; k < 8; ++k)
    c.data[63 - k] = static_cast<uint8_t>(c.bitlen >> (8 * k));
  Sha256Transform(c, c.data);
  for (int k = 0; k < 4; ++k) {
    for (int j = 0; j < 8; ++j) {
      hash[k + j*4] = static_cast<uint8_t>(c.state[j] >> (24 - k*8));
    }
  }
}

std::array<uint8_t, 32> Sha256(std::string_view data) {
  Sha256Ctx c;
  Sha256Init(c);
  Sha256Update(c, reinterpret_cast<const uint8_t*>(data.data()), data.size());
  std::array<uint8_t, 32> out;
  Sha256Final(c, out.data());
  return out;
}

// HMAC-SHA256 per RFC 2104.
std::array<uint8_t, 32> HmacSha256(std::string_view key, std::string_view data) {
  constexpr size_t kBlock = 64;
  uint8_t k[kBlock] = {0};
  if (key.size() > kBlock) {
    auto h = Sha256(key);
    std::memcpy(k, h.data(), h.size());
  } else {
    std::memcpy(k, key.data(), key.size());
  }
  uint8_t ipad[kBlock], opad[kBlock];
  for (size_t i = 0; i < kBlock; ++i) { ipad[i] = k[i] ^ 0x36; opad[i] = k[i] ^ 0x5c; }
  // inner = SHA256(ipad || data)
  Sha256Ctx c;
  Sha256Init(c);
  Sha256Update(c, ipad, kBlock);
  Sha256Update(c, reinterpret_cast<const uint8_t*>(data.data()), data.size());
  uint8_t inner_hash[32];
  Sha256Final(c, inner_hash);
  // outer = SHA256(opad || inner)
  Sha256Init(c);
  Sha256Update(c, opad, kBlock);
  Sha256Update(c, inner_hash, 32);
  std::array<uint8_t, 32> out;
  Sha256Final(c, out.data());
  return out;
}

// Standard base64 encode (no line wrapping).
std::string Base64(const uint8_t* data, size_t len) {
  static const char kAlpha[] =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
  std::string out;
  out.reserve(((len + 2) / 3) * 4);
  for (size_t i = 0; i < len; i += 3) {
    uint32_t v = static_cast<uint32_t>(data[i]) << 16;
    if (i + 1 < len) v |= static_cast<uint32_t>(data[i+1]) << 8;
    if (i + 2 < len) v |= static_cast<uint32_t>(data[i+2]);
    out.push_back(kAlpha[(v >> 18) & 0x3F]);
    out.push_back(kAlpha[(v >> 12) & 0x3F]);
    out.push_back(i + 1 < len ? kAlpha[(v >> 6) & 0x3F] : '=');
    out.push_back(i + 2 < len ? kAlpha[v & 0x3F]       : '=');
  }
  return out;
}

}  // namespace

std::string Sha256Raw(std::string_view data) {
  auto h = Sha256(data);
  return std::string(reinterpret_cast<const char*>(h.data()), h.size());
}

std::string HmacSha256Base64(std::string_view key, std::string_view data) {
  auto h = HmacSha256(key, data);
  return Base64(h.data(), h.size());
}

}  // namespace agentflow::workflow
