...
std::string StreamAccumulator::Feed(std::string_view frame_json) {
  ...
  if (delta.contains("tool_calls") && delta["tool_calls"].is_array()) {
    for (const auto& tc : delta["tool_calls"]) {
      ...
      // malformed frame must never corrupt a valid tool call — losing the bad
      // entry is strictly better than dispatching a tool with wrecked args
      // under someone else's tool_call_id.
      if (tc.contains("index") && !tc["index"].is_number_integer()) continue;
      const int index =
          tc.contains("index") ? tc["index"].get<int>() : 0;
