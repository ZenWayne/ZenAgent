...
2. **工具命名**：按 server 前缀命名空间，注册名为 `mcp__id__toolname`（如 `mcp__fs__read_file`），即约定俗成的 MCP 工具名格式。`__` 为分隔符，因此 server `id` 不能含 `__`。
...
workflow.json
  ...
  └─ agents: { name: { tools: ["mcp__fs__read_file", "get_time", ...] } }
        │
        ▼
WorkflowLoader::LoadAndAttach(json, registry&, opts)   ← 新增协程入口
  ...
  2. 逐个 co_await registry.AttachMcpServer(spec, /*prefix=*/id)
        ├─ 成功 → 工具以 "mcp__id__toolname" 注册进共享 registry
...
message WorkflowSpec {
  ...
  message McpServerDecl {
    string id          = 1;   // 命名空间前缀；需唯一、非空、不含 '__'
...
```json
{
  ...
  "agents": {
    "assistant": {
      ...
      "tools": ["mcp__fs__read_file", "mcp__fs__list_directory", "get_time"]
    }
  },
  "main": "assistant"
}
...
- 注册键改为：`name_prefix.empty() ? schema.name : absl::StrCat("mcp__", name_prefix, "__", schema.name)`。做法：拷一份 `ToolSchema`，把 `.name` 改成带前缀的，作为 `McpToolAdapter` 的 `cached_schema` 传入；`McpToolAdapter::Schema().name`（registry 的键）即带前缀，而 `Invoke` 仍按原始 `remote_tool_name` 调 `client->CallTool`。
...
1. **解析**：JSON → `WorkflowSpec`（复用现有解析逻辑；新增 `mcp_servers` 数组解析）。`id` 校验：非空、唯一、不含 `__`，否则 `InvalidArgumentError`。
2. **连接**：对每个 `decl`，`co_await registry.AttachMcpServer(decl.spec(), decl.id())`：
   - 返回 OK → 工具以 `mcp__id__tool` 注册。
...
3. **引用校验（降级规则）**：构建已声明 server id 集合 `declared_ids`。对每个 agent 的每个 `tools[]` 名字 `t`：
   - `registry.Has(t)` → 保留。
   - 否则从 `mcp__<id>__<remote>` 解析出前缀 `p`（去掉 `mcp__` 后取下一个 `__` 之前；非此格式则视为原生名）：
...
- 未声明前缀 → 硬报错（`LoadAndAttach` 空 server 列表 + agent 引用 `mcp__ghost__echo`）。
...
- 端到端 `LoadAndAttach`（JSON 声明 echo server）→ `registry.Has("mcp__echo__echo")`、workflow 构建成功、`ShutdownMcp` 收尾。
